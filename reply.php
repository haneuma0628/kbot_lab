...
// メッセージ取得
$messages = yaml_parse_file("/var/www/html/kbot_lab/message_reply.yml");

// last_tweet_idから最後に返信したツイートのIDを取得
$handle = fopen("/var/www/html/kbot_lab/last_tweet_id", "r");
if ($handle) {
...
if ($lastTweetId < $mentionId) {
  $handle = fopen("/var/www/html/kbot_lab/last_tweet_id", "w");
  if ($handle) {
